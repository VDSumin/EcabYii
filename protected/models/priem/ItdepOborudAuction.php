...
/**
 * This is the model class for table "itdep_oborud_auction".
 *
 * The followings are the available columns in table 'itdep_oborud_auction':
 * @property integer $id
 ...
 */
class ItdepOborudAuction extends CActiveRecord
{
	/**
	 ...
	 * @return ItdepOborudAuction the static model class
	 */
    ...
    {
        $return = ItdepOborudRequest::model()->findAllByAttributes(['auction' => $id]);
        return count($return);
    }

    public function bigButton($id)
    {
        $return = CHtml::link('Просмотреть', array('/zak/oborudrequest/index', 'id' => $id), array('class' => 'btn btn-info'));
        return $return;
    }

    public function printButton($id)
    {
        $return = CHtml::link('Выгрузить в xls', array('/zak/oborudrequest/print', 'id' => $id), array('class' => 'btn btn-info'));
        return $return;
    }

    public static function getActionStatus($id) {
        $return = ItdepOborudAuction::model()->findByPk($id)->status;
    ...
    {
        $model = ItdepOborudAuction::model()->findAllByAttributes(array('status' => [0,1]));
        $return =[];
