...
<a href="index.php?r=attendance/index"><button class="btn btn-info btn-sm">Список групп</button></a>
<center><h1>Заполнение журнала посещаемости для группы <?php echo $this->getgroupname($group);?></h1></center>
        ...
        <table class=" table table-striped _table-ulist static table-hover" style="table-layout: fixed; width: 100%;">
            ...
            <tr>
                ...
                foreach ($discipline as $dis) {
                    echo '<th colspan="2" style="border-left: 1px solid darkgray ">' . $dis['discipline'] . '<br/>' . '(' . $dis['Type'];
                    $i = 0;
                    while ($i < 10) {
                        $i++;
            ...
            <?php
                foreach ($list as $li) {
                        echo '<tr><th style="width:200px;">' . $li['fio'] . '</th>';
                    $i = 0;
                    foreach ($discipline as $dis) {
                        ...
                        if (in_array($this->markteach($li['fnpp'], $dis['id']), [1, 6])) {
                            ...
                            if (in_array($this->markteach($li['fnpp'], $dis['id']), [2, 4, 5])) {
                        ...
                        $markName = $droplist[$this->markteach($li['fnpp'], $dis['id'])];
                        echo "<div name='droplistmark'>";
                        echo CHtml::dropDownList('listname['. $markid .']', $this->markteach($li['fnpp'], $dis['id']),
                                $droplist,
                                ['class' => 'form-control',
